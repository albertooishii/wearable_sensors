...
// GATT Services Catalog - Agnóstico (sin Material Design)
//
// Carga y gestiona el catálogo de servicios GATT desde JSON.
// Retorna GattService objects con toda la información descriptiva.
//
...
import 'models/gatt_service.dart';

/// Catálogo completo de servicios GATT cargado desde JSON
///
/// Responsable de:
...
///
/// **Agnóstico**: Retorna GattService (sin Material Design).
/// dream_incubator (u otros proyectos) convierten a GattServiceUI/etc. según necesiten.
class GattServicesCatalog {
  /// Caché de servicios cargados
  static Map<String, GattService>? _services;
  ...
  static Future<void> _loadCatalog() async {
    ...
    try {
      ...
      for (final entry in servicesData.entries) {
        _services![entry.key] = GattService.fromJson(
          entry.key,
      ...
      debugPrint(
        '✅ GATT Services Catalog loaded: ${_services!.length} services, ${_categories!.length} categories',
      );
    } on Exception catch (e) {
      debugPrint('❌ Error loading GATT Services Catalog: $e');
      _services = {};
      _categories = {};
    }
  }
  ...
  ///
  /// Ejemplo: getService('180D') → GattService(Heart Rate)
  static Future<GattService?> getService(final String shortUuid) async {
    await _loadCatalog();
    return _services![shortUuid.toUpperCase()];
  }

  /// Obtener todos los servicios
  static Future<Map<String, GattService>> getAllServices() async {
    await _loadCatalog();
    return Map.from(_services!);
  }

  /// Obtener servicios por categoría
  static Future<List<GattService>> getServicesByCategory(
